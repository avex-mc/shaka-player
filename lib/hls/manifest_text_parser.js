...
shaka.hls.ManifestTextParser = class {
  ...
  parsePlaylist(data) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (tag.name == 'EXT-X-STREAM-INF') {
        ...
        const tagUri = this.akamaiAttribute('URI', next);
   ...
   *
   * aka_me_session_id には + が含まれる事があるが、url query の中の + はスペースを意味するので %20 に変換されてしまう
   * 本来は Workers で + を %2B に変換してマニフェストに記入するのが正しいが、Shaka Player 側で aka_me_session_id のクエリだけスペースを + に変換するようにしている
   *
   * @param {string} key
   * @param {string} next
   * @return {any}
   */
  akamaiAttribute(key, next) {
