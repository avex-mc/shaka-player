...
shaka.hls.ManifestTextParser = class {
  ...
  parsePlaylist(data) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (tag.name == 'EXT-X-STREAM-INF') {
        ...
        const tagUri = this.akamaiAttribute_('URI', next);
  ...
  /**
   * Akamai Session ID ( aka_me_session_id ) が GET クエリに含まれる場合、それに応じた属性値を返す
   *
   * aka_me_session_id には + が含まれる事があるが、
   * url query の中の + はスペースを意味するので %20 に変換されてしまう
   * 本来は Workers で + を %2B に変換してマニフェストに記入するのが正しいが、
   * Shaka Player 側で aka_me_session_id のクエリだけスペースを + に変換するようにしている
   *
   * @param {string} key
   * @param {string} next
   * @return {!shaka.hls.Attribute}
   * @private
   */
  akamaiAttribute_(key, next) {
    const uriObj = new goog.Uri(next);
    const queryData = uriObj.getQueryData();
    const ids = queryData.get('aka_me_session_id');
    if (ids.length) {
      queryData.set('aka_me_session_id', ids[0].replace(/ /g, '+'));
      uriObj.setQueryData(queryData);
    }
    const uri = ids.length ? uriObj.toString() : next;
    return new shaka.hls.Attribute(key, uri);
  }
