...
shaka.hls.ManifestTextParser = class {
  ...
  parsePlaylist(data) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (tag.name == 'EXT-X-STREAM-INF') {
        ...
        uri = this.akamaiAttributeUri(next);
        const tagUri = new shaka.hls.Attribute('URI', uri);
  ...
  /**
   * Akamai Session ID が GET クエリに含まれる場合、それに応じた URI 文字列を返す
   * この時、文字列に含まれるスペースを + に変換する ( 置換意図はエビデンスが残っていないため不明 )
   *
   * @param {string} next
   * @return {string}
   */
  akamaiAttributeUri(next) {
    ...
    return ids.length ? uriObj.toString() : next;
  }
